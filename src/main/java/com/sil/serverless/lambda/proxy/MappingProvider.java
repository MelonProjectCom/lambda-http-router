...
import com.sil.serverless.lambda.exception.RoutePreparationException;
import com.sil.serverless.lambda.exception.RouteReflectionException;
import com.sil.serverless.lambda.proxy.convert.InputMessageConverter;
import com.sil.serverless.lambda.proxy.convert.OutputMessageConverter;
import com.sil.serverless.lambda.proxy.convert.ParameterDetails;
import com.sil.serverless.lambda.proxy.utils.AnnotationUtils;
...
import static com.sil.serverless.lambda.proxy.utils.AnnotationUtils.getClassLevelPath;
import static com.sil.serverless.lambda.proxy.utils.PathUtils.getPath;
...
public class MappingProvider<T,R>{
        ...
        return Arrays.stream(beanNamesForType)
                ...
                .collect(Collectors.toMap(
                        bean -> new MethodPath(
                                ...
                                getPath(getClassLevelPath(bean.getClass()), "")),
                        bean -> bean
                ));
    }
    ...
    private Map<MethodPath, Function<T, R>> getMappingToMethods() {
        ...
        getLambdaControllerBeans().values().forEach( bean -> {
            final String classPath = getClassLevelPath(bean.getClass());
    ...
    private Map<MethodPath, Function<T,R>> getAnnotatedFunctions(Class<?> clazz, String classPath) {
        ...
        return Arrays.stream(clazz.getMethods())
                ...
                .map(Optional::get)
                .collect(Collectors.toMap(
                        methodMapping -> new MethodPath(methodMapping.requestMethod(), getPath(classPath, methodMapping.path())),
    ...
    private Map<MethodPath, Function<T,R>> getAnnotatedMethods(Class<?> clazz, String classPath) {
        ...
        return Arrays.stream(clazz.getMethods())
                ...
                .collect(Collectors.toMap(
                        methodMapping -> new MethodPath(methodMapping.requestMethod(), getPath(classPath, methodMapping.path())),
                        methodMapping -> toFunctionFromMethod(methodMapping.method(), clazz, getPath(classPath, methodMapping.path()))
                ));
    }
