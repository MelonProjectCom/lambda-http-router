...
import com.melon.project.serverless.lambda.bind.annotation.Header;
import com.melon.project.serverless.lambda.bind.annotation.PathMapping;
import com.melon.project.serverless.lambda.bind.annotation.PathParameter;
...
import com.melon.project.serverless.lambda.proxy.utils.ParameterValidationUtils;
...
import static com.melon.project.serverless.lambda.proxy.convert.ParameterType.*;
    ...
    public List<ParameterDetails> getInputParameters(Method method) {
        ...
        for (Parameter parameter : method.getParameters()) {
            if(isEventType(parameter.getType())){
                ...
                if(AnnotationUtils.isBody(parameter)){
                    inputs.add(new ParameterDetails(BODY, parameter.getType()));
                } else if (AnnotationUtils.isHeader(parameter)){
                    ParameterValidationUtils.validateParameterType(method, parameter);
                    inputs.add(prepareParameterDetails(HEADER, parameter, parameter.getAnnotation(Header.class).name()));
                } else if ( AnnotationUtils.isHeaders(parameter)){
                    ParameterValidationUtils.validateHeadersParameterType(method, parameter);
                    inputs.add(prepareHeadersDetails(parameter));
                }else if (AnnotationUtils.isPathParameter(parameter)){
                    ParameterValidationUtils.validateParameterType(method, parameter);
                    inputs.add(prepareParameterDetails(PATH_PARAM, parameter, parameter.getAnnotation(PathParameter.class).name()));
                }else if (AnnotationUtils.isQueryParameter(parameter)){
                    ParameterValidationUtils.validateParameterType(method, parameter);
